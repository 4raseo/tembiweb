...
export default function Header() { 
  ...
  return (
    <>
      {/* Navbar Utama */}
      <header className="fixed top-0 left-0 right-0 z-40 bg-black bg-opacity-30 text-white">
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-gray-900 text-white z-50 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="p-6">
          <ul className="flex flex-col space-y-4">
            {navLinks.map((link) => (
              <li key={link.name}><a href={link.href} className="text-lg text-gray-300 hover:text-white transition-colors">{link.name}</a></li>
            ))}
          </ul>
        </nav>
      </aside>
